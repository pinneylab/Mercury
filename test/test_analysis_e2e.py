...
def test_analysis_e2e_pipeline(reference_csv_dir):
    """
    End-to-end integration test for the HT-BAM analysis pipeline using
    ...
    button_concentrations = transform_data(
        data_objs=[htbam_experiment.get_run("button_quant")],
        expr="(a_luminance / slope)",
        expression_vars={"slope": EGFP_SLOPE},
        output_name="concentration",
    )
    htbam_experiment.set_run("enzyme_concentrations", button_concentrations)
    ...
    standard_experiment_data = htbam_experiment.get_run("NADPH")
    standard_fits = fit_luminance_vs_concentration(standard_experiment_data)
    htbam_experiment.set_run("NADPH_standard", standard_fits)
    ...
    product_concentrations = transform_data(
        data_objs=[
            htbam_experiment.get_run("ADP"),
            htbam_experiment.get_run("NADPH_standard"),
        ],
    ...
    kinetics_concentrations = htbam_experiment.get_run("kinetics_ADP_conc")
    ...
    htbam_experiment.set_run("kinetics_ADP_conc_fits", kinetics_fits)
    htbam_experiment.set_run("fit_points_mask", fit_points_mask)
    ...
    htbam_experiment.set_run(
        "buffer_wells_lower_quartile", buffer_wells_lower_quartile
    )

    corrected_initial_rates = transform_data(
        data_objs=[
            htbam_experiment.get_run("kinetics_ADP_conc_fits"),
            htbam_experiment.get_run("buffer_wells_lower_quartile"),
        ],
    ...
    kinetics_fits_bgsub = htbam_experiment.get_run("kinetics_ADP_conc_fits_bgsub")
    ...
    kinetics_fits_masked = htbam_experiment.get_run(
    ...
    kinetics_fits_final = htbam_experiment.get_run(
        "kinetics_ADP_conc_fits_masked"
    )
    MM_fits, MM_pred_data = fit_initial_rates_vs_concentration_with_function(
        data=kinetics_fits_final, model_func=mm_model
    )
    htbam_experiment.set_run("kinetics_ADP_MM_fits", MM_fits)
    htbam_experiment.set_run("kinetics_ADP_MM_pred_data", MM_pred_data)
    ...
    htbam_experiment.set_run("MM_R2_mask", MM_fits_mask)
    ...
    final_mm_fits = htbam_experiment.get_run("kinetics_ADP_MM_fits_masked")
